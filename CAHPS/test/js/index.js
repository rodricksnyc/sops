$(document).ready(function () {
  ...
  $("a, button, input, [tabIndex='0'], #one, .closeRadio").on(
    'keyup',
    function (e) {
  ...
  if ($(document).innerWidth() <= 767) {
    $(window).on('scroll', function () {
      ...
      $('section').each(function () {
        ...
        if (
          screenBottom > elementTop + $(this).find('.heros').height() / 3 &&
    ...
    $('.landing-panel .form-control.access').focus(function () {
      $('html, body').animate(
        {
          scrollTop: $('.landing-panel .form-control.access').offset().top - 40,
        },
        600
      );
      return false;
  ...
  $('#togglepwd, .togglePassword').keypress(togglepwd).click(togglepwd);
  ...
  if ($(document).innerWidth() >= 768) {
    $(window).on('scroll', function () {
      ...
      if (
        !$('.block50').hasClass('active') ||
        $('.block5').hasClass('active')
      ) {
  ...
  $(
    '.menu-buttons-list li, .menu-buttons-floating-list li, .menu-buttons-768-list li'
  ).on('click', function (e) {
    e.preventDefault();
